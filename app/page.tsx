...
type QbrQuestion = { id: string; title: string; question: string; decision: string }
...
const strategicMetrics: MetricItem[] = [
  { id: "M-101", name: "Активные пользователи терминала", description: "Количество уникальных клиентов, регулярно использующих терминал.", strategy: "Активное вовлечение конечного клиента", goal: "Увеличить регулярное использование терминала", plan: "50 тыс.", fact: "52,4 тыс.", progress: 105, status: "green", trend: "+22%", initiatives: 1, source: "DWH" },
  { id: "M-102", name: "Конверсия в целевое действие", description: "Доля пользователей, завершивших ключевое действие в цифровом сервисе.", strategy: "Активное вовлечение конечного клиента", goal: "Повысить конверсию в целевое действие", plan: "18%", fact: "16,3%", progress: 91, status: "yellow", trend: "+2,2 п.п.", initiatives: 2, source: "Вручную" },
  { id: "M-103", name: "Время запуска эксперимента", description: "Среднее время от постановки гипотезы до запуска эксперимента.", strategy: "Современные технологии", goal: "Сократить запуск продуктового эксперимента", plan: "7 дней", fact: "9 дней", progress: 78, status: "red", trend: "−17%", initiatives: 1, source: "BI-витрина" },
  { id: "M-104", name: "Доступность сервиса", description: "Доля времени штатной доступности клиентского сервиса.", strategy: "Современные технологии", goal: "Повысить надежность клиентских сервисов", plan: "99,95%", fact: "99,97%", progress: 100, status: "green", trend: "+0,06 п.п.", initiatives: 1, source: "Мониторинг" },
  { id: "M-105", name: "Новые эмитенты на платформе", description: "Количество новых активных эмитентов, подключенных за период.", strategy: "Развитие рынков капитала", goal: "Увеличить число активных эмитентов", plan: "24", fact: "18", progress: 75, status: "yellow", trend: "+4", initiatives: 0, source: "Вручную" },
  { id: "M-106", name: "Доля автоматизированных операций", description: "Доля операций, выполняемых без ручного участия сотрудников.", strategy: "Современные технологии", goal: "Снизить операционную нагрузку", plan: "65%", fact: "58%", progress: 89, status: "yellow", trend: "+8 п.п.", initiatives: 0, source: "DWH" },
]
...
const initialQbrQuestions: QbrQuestion[] = [
  { id: "Q-101", title: "Дефицит аналитического ресурса", question: "Как закрыть дефицит 0,5 FTE аналитика для продуктовых экспериментов?", decision: "" },
  { id: "Q-102", title: "Задержка миграции витрины данных", question: "Подтверждаем ли поэтапную поставку витрин совместно с Data Platform?", decision: "" },
  { id: "Q-103", title: "Приоритет автоматизации операций", question: "Какой приоритет закрепить за автоматизацией в следующем цикле?", decision: "Автоматизация операций получает приоритет P1." },
]
...
function InitiativeRiskSummary({ risks }: { risks: InitiativeRisk[] }) {
  return <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-rose-700" title={`Всего рисков: ${risks.length}`}><AlertTriangle className="size-6 fill-rose-100 text-rose-300" /><span>{risks.length}</span></span>
}

function InitiativesTable({ items, editable = false, onRemove, onEditFte, onAddRisk, onStatusChange, onEditOwnerTeam, onEdit }: { items: InitiativeItem[]; editable?: boolean; onRemove?: (id: string) => void; onEditFte?: (id: string) => void; onAddRisk?: (id: string) => void; onStatusChange?: (id: string, status: string) => void; onEditOwnerTeam?: (id: string) => void; onEdit?: (item: InitiativeItem) => void }) {
  const showActions = editable || Boolean(onEdit)
  return (
    <div className="overflow-x-auto">
      <Table className={`table-fixed ${showActions ? "min-w-[1260px]" : "min-w-[1160px]"}`}>
        <TableHeader><TableRow className="bg-slate-50/70 hover:bg-slate-50/70"><TableHead className="w-[190px] pl-5 text-xs text-slate-600">Инициатива</TableHead><TableHead className="w-[165px] text-xs text-slate-600">Стратегия</TableHead><TableHead className="w-[180px] text-xs text-slate-600">Ключевая метрика</TableHead><TableHead className="w-[125px] text-xs text-slate-600">Статус</TableHead><TableHead className="w-[105px] text-xs text-slate-600">Прогресс</TableHead><TableHead className="w-[95px] text-xs text-slate-600">Риски</TableHead><TableHead className="w-[95px] text-xs text-slate-600">FTE</TableHead><TableHead className="w-[165px] pr-5 text-xs text-slate-600">Владелец / команда</TableHead>{showActions && <TableHead className="w-[110px] pr-5 text-right text-xs text-slate-600">Действия</TableHead>}</TableRow></TableHeader>
        <TableBody>{items.map((item) => <TableRow key={item.id} className="border-slate-100 align-top"><TableCell className="whitespace-normal break-words py-4 pl-5"><div className="flex items-start gap-2"><div className="min-w-0"><p className="font-medium leading-5 text-slate-950">{item.title}</p><p className="mt-1 text-xs text-slate-400">{item.id}</p></div>{item.trackerUrl && <a href={item.trackerUrl} target="_blank" rel="noreferrer" className="mt-0.5 shrink-0 rounded-md p-1 text-blue-600 hover:bg-blue-50" aria-label="Открыть инициативу в трекере"><ExternalLink className="size-3.5" /></a>}</div></TableCell><TableCell className="whitespace-normal break-words"><p className="text-xs font-medium leading-5 text-slate-700">{item.goal}</p></TableCell><TableCell className="whitespace-normal break-words"><div className="space-y-1">{item.linkedMetrics.map((metric) => <p key={metric} className="text-xs font-medium leading-4 text-slate-700">{metric}</p>)}</div><p className="mt-2 text-xs leading-4 text-slate-400">Цель: {item.target}<br />Сейчас: {item.current}</p></TableCell><TableCell>{editable ? <Select value={item.status} onValueChange={(value) => onStatusChange?.(item.id, value)}><SelectTrigger size="sm" className="w-full text-xs text-slate-800"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="Не начато">Не начато</SelectItem><SelectItem value="В работе">В работе</SelectItem><SelectItem value="Пауза">Пауза</SelectItem><SelectItem value="Завершена">Завершена</SelectItem></SelectContent></Select> : <StatusPill label={item.status} tone={item.tone} />}</TableCell><TableCell><div className="flex items-center gap-2"><Progress value={item.progress} className="h-1.5 w-12 bg-slate-100 [&_[data-slot=progress-indicator]]:bg-slate-800" /><span className="text-xs font-semibold text-slate-700">{item.progress}%</span></div></TableCell><TableCell><div className="flex items-center gap-1.5">{editable ? <RiskCounters risks={item.risks} /> : <InitiativeRiskSummary risks={item.risks} />}{editable && <Button onClick={() => onAddRisk?.(item.id)} variant="ghost" size="icon-sm" className="size-7 text-slate-500" aria-label="Добавить риск"><Plus className="size-3.5" /></Button>}</div></TableCell><TableCell><div className="flex items-center gap-1"><span className="rounded-lg bg-blue-50 px-2 py-1 text-xs font-semibold text-blue-700">{item.fte.toFixed(1).replace(".", ",")}</span>{editable && <Button onClick={() => onEditFte?.(item.id)} variant="ghost" size="icon-sm" className="size-7 text-slate-500" aria-label="Изменить состав"><Pencil className="size-3.5" /></Button>}</div></TableCell><TableCell className="whitespace-normal break-words pr-5"><div className="flex items-start gap-1"><div><p className="text-xs font-medium leading-5 text-slate-700">{item.owner}</p><p className="mt-1 text-xs leading-4 text-slate-400">{item.team}</p></div>{editable && <Button onClick={() => onEditOwnerTeam?.(item.id)} variant="ghost" size="icon-sm" className="size-7 text-slate-500" aria-label="Изменить владельца и команду"><Pencil className="size-3.5" /></Button>}</div></TableCell>{showActions && <TableCell className="pr-5 text-right"><div className="flex justify-end gap-1">{onEdit && <Button onClick={() => onEdit(item)} variant="ghost" size="icon-sm" className="text-slate-500 hover:text-blue-700" aria-label="Редактировать инициативу"><Pencil className="size-4" /></Button>}{editable && <Button onClick={() => onRemove?.(item.id)} variant="ghost" size="icon-sm" className="text-slate-400 hover:text-rose-600" aria-label="Удалить инициативу из QBR"><Trash2 className="size-4" /></Button>}</div></TableCell>}</TableRow>)}</TableBody>
      </Table>
    </div>
  )
}

function InitiativesPage({ items, setItems, metricCatalog, teams }: { items: InitiativeItem[]; setItems: (items: InitiativeItem[]) => void; metricCatalog: MetricItem[]; teams: TeamItem[] }) {
  ...
  const filteredItems = items.filter((item) => (!query || [item.title, item.id, item.owner, item.team].some((value) => value.toLowerCase().includes(query))) && (goalFilter === "all" || item.goal === goalFilter) && (metricFilter === "all" || item.linkedMetrics.includes(metricFilter)) && (statusFilter === "all" || item.status === statusFilter) && (ownerFilter === "all" || item.owner === ownerFilter) && (teamFilter === "all" || item.team === teamFilter))

  const resetDrafts = () => {
    setEditingId(null); setTitleDraft(""); setGoalDraft(""); setMetricsDraft([]); setOwnerDraft(""); setTeamDraft(""); setFteDraft("0"); setTrackerDraft("")
  }
  const openCreate = () => { resetDrafts(); setDialogOpen(true) }
  const openEdit = (item: InitiativeItem) => {
    setEditingId(item.id); setTitleDraft(item.title); setGoalDraft(item.goal); setMetricsDraft(item.linkedMetrics); setOwnerDraft(item.owner); setTeamDraft(item.team); setFteDraft(String(item.fte)); setTrackerDraft(item.trackerUrl ?? ""); setDialogOpen(true)
  }
  const saveInitiative = () => {
    ...
    const common = { title: titleDraft.trim(), strategy: firstMetric?.strategy ?? "Стратегия компании", goal: goalDraft, metric: metricsDraft.join(", "), linkedMetrics: metricsDraft, owner: ownerDraft, team: team.name, fte: Number(fteDraft.replace(",", ".")) || 0, trackerUrl: trackerDraft.trim() || undefined }
    if (editingId) {
      setItems(items.map((item) => item.id === editingId ? { ...item, ...common } : item))
    } else {
      setItems([...items, { id: `IN-${260 + items.length}`, ...common, target: "—", current: "—", status: "Не начато", tone: "slate", progress: 0, employees: team.employees.map((employee) => employee.name), risks: [] }])
    }
    setDialogOpen(false); resetDrafts()
  }
  return (
    <main className="mx-auto w-full max-w-[1480px] p-4 md:p-7">
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-5"><div><h2 className="text-xl font-semibold text-slate-950">Портфель инициатив</h2><p className="mt-1 text-sm text-slate-500">Работы, прогресс и статусы</p></div><Button onClick={openCreate} className="bg-[#ef3e42] text-white hover:bg-[#d92f34]"><Plus />Добавить инициативу</Button></div>
        <div className="border-b border-slate-100 bg-slate-50/70 p-4">
          <div className="relative"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-400" /><Input value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="Быстрый поиск по инициативам" className="bg-white pl-9 text-slate-900 placeholder:text-slate-400" /></div>
          <div className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-5">{[{ value: goalFilter, set: setGoalFilter, label: "Стратегическая цель", options: strategicGoals }, { value: metricFilter, set: setMetricFilter, label: "Метрика", options: metricCatalog.map((metric) => metric.name) }, { value: statusFilter, set: setStatusFilter, label: "Статус", options: ["Не начато", "В работе", "Пауза", "Завершена"] }, { value: ownerFilter, set: setOwnerFilter, label: "Владелец", options: owners }, { value: teamFilter, set: setTeamFilter, label: "Команда", options: teams.map((team) => team.name) }].map((filter) => <Select key={filter.label} value={filter.value} onValueChange={filter.set}><SelectTrigger className="w-full border-slate-300 bg-white text-xs font-medium text-slate-800"><SelectValue placeholder={filter.label} /></SelectTrigger><SelectContent><SelectItem value="all">Все · {filter.label}</SelectItem>{filter.options.map((option) => <SelectItem key={option} value={option}>{option}</SelectItem>)}</SelectContent></Select>)}</div>
        </div>
        <InitiativesTable items={filteredItems} onEdit={openEdit} />
      </section>

      <Dialog open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open) resetDrafts() }}><DialogContent className="sm:max-w-2xl"><DialogHeader><DialogTitle>{editingId ? "Редактировать инициативу" : "Новая инициатива"}</DialogTitle><DialogDescription>Свяжите инициативу со стратегической целью, метриками и ответственной командой.</DialogDescription></DialogHeader><div className="grid gap-4 sm:grid-cols-2"><div className="space-y-2 sm:col-span-2"><Label htmlFor="initiative-title">Название инициативы</Label><Input id="initiative-title" value={titleDraft} onChange={(event) => setTitleDraft(event.target.value)} placeholder="Например: Автоматизация проверки эмитентов" /></div><div className="space-y-2 sm:col-span-2"><Label>Стратегическая цель</Label><Select value={goalDraft} onValueChange={setGoalDraft}><SelectTrigger className="w-full"><SelectValue placeholder="Выберите одну цель" /></SelectTrigger><SelectContent>{strategicGoals.map((goal) => <SelectItem key={goal} value={goal}>{goal}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2 sm:col-span-2"><Label>Стратегические метрики</Label><DropdownMenu><DropdownMenuTrigger asChild><Button variant="outline" className="w-full justify-between font-normal">{metricsDraft.length ? `Выбрано: ${metricsDraft.length}` : "Выберите одну или несколько метрик"}<ChevronRight className="size-4 rotate-90" /></Button></DropdownMenuTrigger><DropdownMenuContent className="w-[420px] max-w-[calc(100vw-2rem)]">{metricCatalog.map((metric) => <DropdownMenuCheckboxItem key={metric.id} checked={metricsDraft.includes(metric.name)} onCheckedChange={(checked) => setMetricsDraft(checked ? [...metricsDraft, metric.name] : metricsDraft.filter((item) => item !== metric.name))} onSelect={(event) => event.preventDefault()}>{metric.name}</DropdownMenuCheckboxItem>)}</DropdownMenuContent></DropdownMenu></div><div className="space-y-2"><Label>Команда</Label><Select value={teamDraft} onValueChange={(value) => { const team = teams.find((item) => item.name === value); setTeamDraft(value); setFteDraft(String(team?.employees.length ?? 0)); if (team && !team.employees.some((employee) => employee.name === ownerDraft)) setOwnerDraft(team.employees[0]?.name ?? "") }}><SelectTrigger className="w-full"><SelectValue placeholder="Выберите команду" /></SelectTrigger><SelectContent>{teams.map((team) => <SelectItem key={team.id} value={team.name}>{team.name}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><Label>Владелец</Label><Select value={ownerDraft} onValueChange={setOwnerDraft}><SelectTrigger className="w-full"><SelectValue placeholder="Выберите владельца" /></SelectTrigger><SelectContent>{owners.map((owner) => <SelectItem key={owner} value={owner}>{owner}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><Label htmlFor="initiative-fte">FTE</Label><Input id="initiative-fte" inputMode="decimal" value={fteDraft} onChange={(event) => setFteDraft(event.target.value)} /></div><div className="space-y-2"><Label htmlFor="initiative-tracker">Ссылка на трекер</Label><Input id="initiative-tracker" type="url" value={trackerDraft} onChange={(event) => setTrackerDraft(event.target.value)} placeholder="https://tracker/IN-123" /></div></div><DialogFooter><Button variant="outline" onClick={() => setDialogOpen(false)}>Отмена</Button><Button onClick={saveInitiative} disabled={!titleDraft.trim() || !goalDraft || !metricsDraft.length || !ownerDraft || !teamDraft}>{editingId ? "Сохранить изменения" : "Добавить инициативу"}</Button></DialogFooter></DialogContent></Dialog>
    </main>
  )
}
...
function MetricsPage({ items, setItems }: { items: MetricItem[]; setItems: (items: MetricItem[]) => void }) {
  ...
  const [searchQuery, setSearchQuery] = useState("")
  const query = searchQuery.trim().toLowerCase()
  const filteredItems = items.filter((metric) => !query || [metric.name, metric.description, metric.source].some((value) => value.toLowerCase().includes(query)))
  const resetDrafts = () => { setEditingId(null); setNameDraft(""); setDescriptionDraft(""); setValueDraft(""); setSourceMode("manual") }
  const openCreate = () => { resetDrafts(); setDialogOpen(true) }
  const openEdit = (metric: MetricItem) => {
    setEditingId(metric.id); setNameDraft(metric.name); setDescriptionDraft(metric.description); setValueDraft(metric.fact); setSourceMode(metric.source === "Вручную" ? "manual" : "integration"); setDialogOpen(true)
  }
  const saveMetric = () => {
    if (!nameDraft.trim() || !descriptionDraft.trim() || (sourceMode === "manual" && !valueDraft.trim())) return
    if (editingId) {
      setItems(items.map((metric) => metric.id === editingId ? { ...metric, name: nameDraft.trim(), description: descriptionDraft.trim(), fact: sourceMode === "manual" ? valueDraft.trim() : metric.fact, plan: sourceMode === "manual" ? valueDraft.trim() : metric.plan, source: sourceMode === "manual" ? "Вручную" : metric.source } : metric))
    } else {
      setItems([...items, { id: `M-${110 + items.length}`, name: nameDraft.trim(), description: descriptionDraft.trim(), strategy: "", goal: nameDraft.trim(), plan: valueDraft.trim(), fact: valueDraft.trim(), progress: 100, status: "green", trend: "", initiatives: 0, source: "Вручную" }])
    }
    setDialogOpen(false); resetDrafts()
  }
  return <main className="mx-auto w-full max-w-[1480px] p-4 md:p-7"><section className="overflow-hidden rounded-2xl border border-slate-200 bg-white"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-5"><div><h2 className="text-xl font-semibold text-slate-950">Справочник метрик</h2><p className="mt-1 text-sm text-slate-500">Единый каталог показателей для инициатив и квартальных обзоров.</p></div><Button onClick={openCreate} className="bg-[#ef3e42] text-white hover:bg-[#d92f34]"><Plus />Добавить метрику</Button></div><div className="border-b border-slate-100 bg-slate-50/70 p-4"><div className="relative"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-400" /><Input value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="Быстрый поиск по метрикам" className="bg-white pl-9" /></div></div><div className="overflow-x-auto"><Table className="min-w-[980px]"><TableHeader><TableRow className="bg-slate-50/70 hover:bg-slate-50/70"><TableHead className="w-[25%] pl-5 text-xs text-slate-600">Метрика</TableHead><TableHead className="w-[38%] text-xs text-slate-600">Описание</TableHead><TableHead className="w-[15%] text-xs text-slate-600">Значение</TableHead><TableHead className="w-[15%] text-xs text-slate-600">Источник</TableHead><TableHead className="w-[7%] pr-5 text-right text-xs text-slate-600">Действия</TableHead></TableRow></TableHeader><TableBody>{filteredItems.map((metric) => <TableRow key={metric.id} className="border-slate-100"><TableCell className="py-4 pl-5"><p className="font-medium text-slate-900">{metric.name}</p><p className="mt-1 text-xs text-slate-400">{metric.id}</p></TableCell><TableCell className="text-sm leading-6 text-slate-600">{metric.description}</TableCell><TableCell><p className="text-lg font-semibold text-slate-950">{metric.fact}</p></TableCell><TableCell><span className="inline-flex items-center gap-1.5 rounded-lg bg-slate-100 px-2.5 py-1.5 text-xs font-medium text-slate-700"><Database className="size-3.5" />{metric.source}</span></TableCell><TableCell className="pr-5 text-right"><Button onClick={() => openEdit(metric)} variant="ghost" size="icon-sm" className="text-slate-500 hover:text-blue-700" aria-label="Редактировать метрику"><Pencil className="size-4" /></Button></TableCell></TableRow>)}</TableBody></Table></div></section><Dialog open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open) resetDrafts() }}><DialogContent><DialogHeader><DialogTitle>{editingId ? "Редактировать метрику" : "Добавить метрику"}</DialogTitle><DialogDescription>Укажите название, описание, значение и способ получения данных.</DialogDescription></DialogHeader><div className="space-y-4"><div className="space-y-2"><Label htmlFor="new-metric-name">Метрика</Label><Input id="new-metric-name" value={nameDraft} onChange={(event) => setNameDraft(event.target.value)} placeholder="Название метрики" /></div><div className="space-y-2"><Label htmlFor="new-metric-description">Описание</Label><Textarea id="new-metric-description" value={descriptionDraft} onChange={(event) => setDescriptionDraft(event.target.value)} placeholder="Что измеряет показатель и как интерпретируется" rows={3} /></div><div className="space-y-2"><Label>Значение и источник</Label><Tabs value={sourceMode} onValueChange={setSourceMode}><TabsList className="grid h-10 w-full grid-cols-2"><TabsTrigger value="manual">Ручной ввод</TabsTrigger><TabsTrigger value="integration">SQL / DWH / ETL / API</TabsTrigger></TabsList><TabsContent value="manual" className="mt-3"><Input value={valueDraft} onChange={(event) => setValueDraft(event.target.value)} placeholder="Например: 18%, 50 тыс. или 7 дней" /></TabsContent><TabsContent value="integration" className="mt-3 rounded-xl border border-dashed border-slate-300 bg-slate-50 p-5 text-sm leading-6 text-slate-500">Возможность добавить источник будет реализована в следующих версиях приложения.</TabsContent></Tabs></div></div><DialogFooter><Button variant="outline" onClick={() => setDialogOpen(false)}>Отмена</Button><Button onClick={saveMetric} disabled={!nameDraft.trim() || !descriptionDraft.trim() || (!editingId && sourceMode !== "manual") || (sourceMode === "manual" && !valueDraft.trim())}>{editingId ? "Сохранить изменения" : "Добавить"}</Button></DialogFooter></DialogContent></Dialog></main>
}
...
function TeamsPage({ teams, setTeams }: { teams: TeamItem[]; setTeams: (teams: TeamItem[]) => void }) {
  ...
  const toggleTeam = (id: string) => setExpandedTeams(expandedTeams.includes(id) ? expandedTeams.filter((item) => item !== id) : [...expandedTeams, id])
  return <main className="mx-auto w-full max-w-[1480px] p-4 md:p-7"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-2xl font-semibold text-slate-950">Команды и сотрудники</h2><p className="mt-1 text-sm text-slate-500">Раскройте строку команды, чтобы посмотреть состав сотрудников.</p></div><div className="flex flex-wrap gap-2"><Button onClick={() => setImportDialogOpen(true)} className="bg-blue-600 text-white shadow-sm hover:bg-blue-700"><Download />Импорт из LDAP-каталога</Button><Button onClick={() => setTeamDialogOpen(true)} className="bg-[#ef3e42] text-white hover:bg-[#d92f34]"><Plus />Добавить команду</Button></div></div><section className="mt-5 overflow-hidden rounded-2xl border border-slate-200 bg-white"><div className="overflow-x-auto"><Table className="min-w-[900px]"><TableHeader><TableRow className="bg-slate-50/80 hover:bg-slate-50/80"><TableHead className="w-[32%] pl-5 text-xs text-slate-600">Команда</TableHead><TableHead className="w-[30%] text-xs text-slate-600">Подразделение</TableHead><TableHead className="w-[14%] text-center text-xs text-slate-600">Сотрудников</TableHead><TableHead className="w-[24%] pr-5 text-right text-xs text-slate-600">Действия</TableHead></TableRow></TableHeader><TableBody>{teams.map((team) => <>{<TableRow key={team.id} onClick={() => toggleTeam(team.id)} className="cursor-pointer border-slate-100 hover:bg-slate-50"><TableCell className="py-5 pl-5"><div className="flex items-center gap-3"><span className="rounded-xl bg-blue-50 p-2.5 text-blue-700"><Building2 className="size-5" /></span><div><p className="font-semibold text-slate-950">{team.name}</p><p className="mt-1 text-xs text-slate-400">{team.id}</p></div></div></TableCell><TableCell className="py-5 text-sm text-slate-600">{team.department}</TableCell><TableCell className="py-5 text-center"><span className="text-2xl font-semibold tracking-tight text-slate-950">{team.employees.length}</span></TableCell><TableCell className="py-5 pr-5"><div className="flex items-center justify-end gap-2"><Button onClick={(event) => { event.stopPropagation(); setEmployeeTeamId(team.id) }} size="sm" className="bg-blue-600 text-white hover:bg-blue-700"><UserPlus />Добавить сотрудника</Button><Button variant="ghost" size="icon-sm" aria-label={expandedTeams.includes(team.id) ? "Свернуть команду" : "Развернуть команду"}>{expandedTeams.includes(team.id) ? <ChevronUp className="size-4" /> : <ChevronDown className="size-4" />}</Button></div></TableCell></TableRow>}{expandedTeams.includes(team.id) && <TableRow key={`${team.id}-employees`} className="bg-slate-50/60 hover:bg-slate-50/60"><TableCell colSpan={4} className="px-5 py-4"><div className="grid gap-2 md:grid-cols-2 xl:grid-cols-3">{team.employees.length ? team.employees.map((employee) => <div key={employee.id} className="flex items-center gap-3 rounded-xl border border-slate-200 bg-white px-3 py-3"><span className="grid size-9 shrink-0 place-items-center rounded-full bg-slate-100 text-[11px] font-semibold text-slate-600">{employee.name.split(" ").map((part) => part[0]).join("")}</span><div className="min-w-0"><p className="truncate text-sm font-medium text-slate-800">{employee.name}</p><p className="mt-0.5 truncate text-xs text-slate-500">{employee.role} · {employee.email}</p></div></div>) : <p className="text-sm text-slate-400">Сотрудники еще не добавлены</p>}</div></TableCell></TableRow>}</>)}</TableBody></Table></div></section><Dialog open={importDialogOpen} onOpenChange={setImportDialogOpen}><DialogContent><DialogHeader><DialogTitle>Импорт из LDAP-каталога</DialogTitle><DialogDescription>Возможность импорта из LDAP-каталога</DialogDescription></DialogHeader><div className="rounded-xl border border-dashed border-blue-200 bg-blue-50 p-5 text-sm leading-6 text-blue-800">Подключение корпоративной службы каталогов будет реализовано в следующих версиях приложения.</div><DialogFooter><Button onClick={() => setImportDialogOpen(false)}>Понятно</Button></DialogFooter></DialogContent></Dialog><Dialog open={teamDialogOpen} onOpenChange={setTeamDialogOpen}><DialogContent><DialogHeader><DialogTitle>Добавить команду</DialogTitle><DialogDescription>Создайте команду в организационном справочнике.</DialogDescription></DialogHeader><div className="space-y-4"><div className="space-y-2"><Label htmlFor="team-name">Название команды</Label><Input id="team-name" value={teamName} onChange={(event) => setTeamName(event.target.value)} /></div><div className="space-y-2"><Label htmlFor="team-department">Подразделение</Label><Input id="team-department" value={department} onChange={(event) => setDepartment(event.target.value)} /></div></div><DialogFooter><Button variant="outline" onClick={() => setTeamDialogOpen(false)}>Отмена</Button><Button onClick={addTeam} disabled={!teamName.trim() || !department.trim()}>Добавить команду</Button></DialogFooter></DialogContent></Dialog><Dialog open={Boolean(employeeTeamId)} onOpenChange={(open) => !open && setEmployeeTeamId(null)}><DialogContent><DialogHeader><DialogTitle>Добавить сотрудника</DialogTitle><DialogDescription>Команда: {selectedTeam?.name}</DialogDescription></DialogHeader><div className="space-y-4"><div className="space-y-2"><Label htmlFor="employee-name">Имя</Label><Input id="employee-name" value={employeeName} onChange={(event) => setEmployeeName(event.target.value)} /></div><div className="space-y-2"><Label htmlFor="employee-role">Должность</Label><Input id="employee-role" value={employeeRole} onChange={(event) => setEmployeeRole(event.target.value)} /></div><div className="space-y-2"><Label htmlFor="employee-email">Email</Label><Input id="employee-email" type="email" value={employeeEmail} onChange={(event) => setEmployeeEmail(event.target.value)} /></div></div><DialogFooter><Button variant="outline" onClick={() => setEmployeeTeamId(null)}>Отмена</Button><Button onClick={addEmployee} disabled={!employeeName.trim() || !employeeRole.trim() || !employeeEmail.trim()} className="bg-blue-600 text-white hover:bg-blue-700">Добавить сотрудника</Button></DialogFooter></DialogContent></Dialog></main>
}
...
function QbrOverview({ mode, metricItems, setMetricItems, metricCatalog, initiativeItems, setInitiativeItems, qbrInitiativeIds, setQbrInitiativeIds, teams, questions, setQuestions, onOpenInitiatives }: { mode: string; metricItems: MetricItem[]; setMetricItems: (items: MetricItem[]) => void; metricCatalog: MetricItem[]; initiativeItems: InitiativeItem[]; setInitiativeItems: (items: InitiativeItem[]) => void; qbrInitiativeIds: string[]; setQbrInitiativeIds: (ids: string[]) => void; teams: TeamItem[]; questions: QbrQuestion[]; setQuestions: (items: QbrQuestion[]) => void; onOpenInitiatives: () => void }) {
  ...
  const addMetric = () => {
    ...
    setMetricItems([...metricItems, { ...catalogMetric, plan: metricTargetDraft || catalogMetric.plan }])
    setMetricDialogOpen(false); setMetricDraftId(""); setMetricTargetDraft("")
  }
  ...
  const addRisk = () => {
    ...
    setRiskInitiativeId(null); setRiskName(""); setRiskDescription(""); setRiskLevel("medium")
  }
  const addQuestion = () => {
    if (!questionTitleDraft.trim() || !questionTextDraft.trim()) return
    setQuestions([...questions, { id: `Q-${Date.now()}`, title: questionTitleDraft.trim(), question: questionTextDraft.trim(), decision: "" }])
    setQuestionDialogOpen(false); setQuestionTitleDraft(""); setQuestionTextDraft("")
  }
  const addStandaloneDecision = () => {
    if (!decisionTitleDraft.trim() || !decisionDraft.trim()) return
    setQuestions([...questions, { id: `D-${Date.now()}`, title: decisionTitleDraft.trim(), question: "Решение без привязки к вопросу", decision: decisionDraft.trim() }])
    setStandaloneDecisionOpen(false); setDecisionTitleDraft(""); setDecisionDraft("")
  }
  ...
  return (
    <main className="mx-auto w-full max-w-[1480px] p-4 md:p-7">
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><StatCard icon={Target} label="Цели и метрики" value={`${metricItems.length}`} note="включено в QBR" tone="blue" /><StatCard icon={Rocket} label="Инициативы" value={`${selectedInitiatives.length}`} note="включено в QBR" tone="amber" /><StatCard icon={BriefcaseBusiness} label="Ресурс инициатив" value={`${selectedInitiatives.reduce((sum, item) => sum + item.fte, 0)} FTE`} note="по составу команд" tone="violet" /><StatCard icon={ListChecks} label="Вопросы" value={`${questions.length}`} note={reviewMode ? "обсуждаются на ревью" : "подготовлено к встрече"} tone="red" /></section>

      <section className="mt-5 overflow-hidden rounded-2xl border border-slate-200 bg-white"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4"><div><h3 className="text-xl font-semibold text-slate-950">Цели и метрики</h3><p className="mt-1 text-sm text-slate-500">Результаты периода и инициативы, которые обеспечивают достижение цели</p></div>{editable ? <Button onClick={() => setMetricDialogOpen(true)} size="sm" className="bg-slate-950 text-white hover:bg-slate-800"><Plus />Добавить метрику</Button> : null}</div><div className="grid gap-4 bg-slate-50/50 p-4 xl:grid-cols-2">{metricItems.map((metric) => { const linked = selectedInitiatives.filter((initiative) => initiative.linkedMetrics.includes(metric.name)); const inheritedRisks = linked.flatMap((initiative) => initiative.risks); return <article key={metric.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.03)]"><div className="flex items-start justify-between gap-3"><div className="flex items-start gap-3"><StatusDot status={metric.status} /><div><h4 className="font-semibold leading-5 text-slate-950">{metric.name}</h4><p className="mt-1 text-xs leading-5 text-slate-500">{metric.description}</p></div></div><RiskCounters risks={inheritedRisks} /></div><div className="mt-4 grid grid-cols-2 gap-3"><div><p className="text-xs text-slate-500">Целевое значение</p>{editable ? <Input value={metric.plan} onChange={(event) => updateMetric(metric.id, { plan: event.target.value })} className="mt-1 h-8" /> : <p className="mt-1 text-lg font-semibold text-slate-950">{metric.plan}</p>}</div><div><p className="text-xs text-slate-500">Фактическое значение</p>{editable ? <Input value={metric.fact} onChange={(event) => updateMetric(metric.id, { fact: event.target.value })} className="mt-1 h-8" /> : <p className="mt-1 text-lg font-semibold text-slate-950">{metric.fact}</p>}</div></div><div className="mt-4 flex items-center gap-3"><Progress value={Math.min(metric.progress, 100)} className="h-2 flex-1 bg-slate-100 [&_[data-slot=progress-indicator]]:bg-slate-900" /><span className="text-sm font-semibold text-slate-700">{metric.progress}%</span></div><div className="mt-4 rounded-xl bg-slate-50 p-3"><p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-slate-400">Связанные инициативы</p><div className="mt-2 flex flex-wrap gap-1.5">{linked.length ? linked.map((initiative) => <span key={initiative.id} className="rounded-full bg-white px-2.5 py-1 text-xs font-medium text-slate-700 ring-1 ring-slate-200">{initiative.id} · {initiative.title}</span>) : <span className="text-xs text-amber-700">Нет связанных инициатив</span>}</div></div><div className="mt-4 flex items-center justify-between border-t border-slate-100 pt-3">{editable ? <Select value={metric.source} onValueChange={(value) => updateMetric(metric.id, { source: value })}><SelectTrigger size="sm" className="w-[160px] text-xs"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="Вручную">Вручную</SelectItem><SelectItem value="DWH">DWH</SelectItem><SelectItem value="BI-витрина">BI-витрина</SelectItem><SelectItem value="Мониторинг">Мониторинг</SelectItem><SelectItem value="Файл AdHoc">Файл AdHoc</SelectItem></SelectContent></Select> : <span className="inline-flex items-center gap-1.5 text-xs text-slate-500"><Database className="size-3.5" />{metric.source}</span>}{editable && <Button onClick={() => setMetricItems(metricItems.filter((item) => item.id !== metric.id))} variant="ghost" size="icon-sm" className="text-slate-400 hover:text-rose-600"><Trash2 className="size-4" /></Button>}</div></article> })}</div></section>

      <section className="mt-5 overflow-hidden rounded-2xl border border-slate-200 bg-white"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4"><div><div className="flex items-center gap-2"><Rocket className="size-5 text-[#ef3e42]" /><h3 className="text-xl font-semibold text-slate-950">Инициативы квартала</h3></div><p className="mt-1 text-sm text-slate-500">Стратегия, ключевая метрика, статус, риски и состав команды</p></div>{editable ? <Button onClick={() => setInitiativeDialogOpen(true)} size="sm" className="bg-slate-950 text-white hover:bg-slate-800"><Plus />Добавить из портфеля</Button> : <Button onClick={onOpenInitiatives} variant="outline" size="sm">Открыть портфель <ArrowRight /></Button>}</div><InitiativesTable items={selectedInitiatives} editable={editable} onRemove={(id) => setQbrInitiativeIds(qbrInitiativeIds.filter((item) => item !== id))} onEditFte={setPeopleInitiativeId} onAddRisk={setRiskInitiativeId} onStatusChange={(id, status) => setInitiativeItems(initiativeItems.map((initiative) => initiative.id === id ? { ...initiative, status, tone: status === "Завершена" ? "green" : status === "Пауза" ? "red" : status === "В работе" ? "blue" : "yellow", progress: status === "Завершена" ? 100 : initiative.progress } : initiative))} onEditOwnerTeam={(id) => { const initiative = initiativeItems.find((item) => item.id === id); setOwnerTeamInitiativeId(id); setOwnerEditDraft(initiative?.owner ?? ""); setTeamEditDraft(initiative?.team ?? "") }} /></section>

      <section className="mt-5 overflow-hidden rounded-2xl border border-slate-200 bg-white"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4"><div><div className="flex items-center gap-2"><ListChecks className="size-5 text-[#ef3e42]" /><h3 className="text-xl font-semibold text-slate-950">Вопросы и решения</h3></div><p className="mt-1 text-sm text-slate-500">{editable ? "Подготовьте вопросы, риски и ограничения к встрече" : "Зафиксируйте решения по вопросам во время ревью"}</p></div>{editable ? <Button onClick={() => setQuestionDialogOpen(true)} size="sm" className="bg-slate-950 text-white hover:bg-slate-800"><Plus />Добавить вопрос</Button> : reviewMode ? <Button onClick={() => setStandaloneDecisionOpen(true)} size="sm" className="bg-violet-600 text-white hover:bg-violet-700"><Plus />Решение без вопроса</Button> : null}</div><div className="divide-y divide-slate-100">{questions.map((item, index) => <article key={item.id} className="grid gap-4 p-5 lg:grid-cols-[36px_minmax(0,1fr)_minmax(240px,0.65fr)] lg:items-start"><span className="grid size-8 place-items-center rounded-full bg-slate-100 text-xs font-semibold text-slate-600">{index + 1}</span><div><h4 className="font-semibold text-slate-950">{item.title}</h4><p className="mt-1 text-xs font-semibold uppercase tracking-[0.1em] text-slate-400">Вопрос</p><p className="mt-1 text-sm leading-6 text-slate-600">{item.question}</p>{editable && <Button onClick={() => setQuestions(questions.filter((question) => question.id !== item.id))} variant="ghost" size="sm" className="mt-2 px-0 text-rose-600"><Trash2 />Удалить</Button>}</div><div className="rounded-xl bg-slate-50 p-3"><p className="text-xs font-semibold uppercase tracking-[0.1em] text-slate-400">Решение</p>{reviewMode ? <Textarea value={item.decision} onChange={(event) => setQuestions(questions.map((question) => question.id === item.id ? { ...question, decision: event.target.value } : question))} placeholder="Зафиксируйте решение на встрече" rows={3} className="mt-2 bg-white text-slate-950 placeholder:text-slate-400" /> : <p className="mt-2 text-sm leading-6 text-slate-600">{item.decision || "Будет зафиксировано во время ревью"}</p>}</div></article>)}</div></section>

      <Dialog open={metricDialogOpen} onOpenChange={setMetricDialogOpen}><DialogContent><DialogHeader><DialogTitle>Добавить целевую метрику</DialogTitle><DialogDescription>Выберите метрику из справочника и задайте целевое значение для квартала.</DialogDescription></DialogHeader><div className="space-y-4"><div className="space-y-2"><Label>Метрика</Label><Select value={metricDraftId} onValueChange={(value) => { const metric = metricCatalog.find((item) => item.id === value); setMetricDraftId(value); setMetricTargetDraft(metric?.plan ?? "") }}><SelectTrigger className="w-full"><SelectValue placeholder="Выберите метрику" /></SelectTrigger><SelectContent>{availableMetrics.map((metric) => <SelectItem key={metric.id} value={metric.id}>{metric.name}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><Label htmlFor="metric-target">Целевое значение</Label><Input id="metric-target" value={metricTargetDraft} onChange={(event) => setMetricTargetDraft(event.target.value)} /></div></div><DialogFooter><Button variant="outline" onClick={() => setMetricDialogOpen(false)}>Отмена</Button><Button onClick={addMetric} disabled={!metricDraftId}>Добавить в QBR</Button></DialogFooter></DialogContent></Dialog>

      <Dialog open={initiativeDialogOpen} onOpenChange={setInitiativeDialogOpen}><DialogContent className="sm:max-w-2xl"><DialogHeader><DialogTitle>Добавить инициативу в QBR</DialogTitle><DialogDescription>Выберите инициативы из портфеля текущего квартала.</DialogDescription></DialogHeader><div className="max-h-[420px] space-y-2 overflow-y-auto">{availableInitiatives.length ? availableInitiatives.map((initiative) => <div key={initiative.id} className="flex items-center gap-3 rounded-xl border border-slate-200 p-3"><div className="min-w-0 flex-1"><p className="text-sm font-semibold text-slate-900">{initiative.title}</p><p className="mt-1 text-xs text-slate-500">{initiative.id} · {initiative.goal}</p></div><Button onClick={() => setQbrInitiativeIds([...qbrInitiativeIds, initiative.id])} variant="outline" size="sm"><Plus />Добавить</Button></div>) : <div className="rounded-xl bg-slate-50 p-6 text-center text-sm text-slate-500">Все инициативы портфеля уже добавлены в QBR.</div>}</div><DialogFooter><Button onClick={() => setInitiativeDialogOpen(false)}>Готово</Button></DialogFooter></DialogContent></Dialog>

      <Dialog open={questionDialogOpen} onOpenChange={setQuestionDialogOpen}><DialogContent><DialogHeader><DialogTitle>Добавить вопрос</DialogTitle><DialogDescription>Сформулируйте вопрос, который необходимо обсудить на QBR.</DialogDescription></DialogHeader><div className="space-y-4"><div className="space-y-2"><Label htmlFor="question-title">Заголовок</Label><Input id="question-title" value={questionTitleDraft} onChange={(event) => setQuestionTitleDraft(event.target.value)} /></div><div className="space-y-2"><Label htmlFor="question-text">Вопрос</Label><Textarea id="question-text" value={questionTextDraft} onChange={(event) => setQuestionTextDraft(event.target.value)} rows={4} /></div></div><DialogFooter><Button variant="outline" onClick={() => setQuestionDialogOpen(false)}>Отмена</Button><Button onClick={addQuestion} disabled={!questionTitleDraft.trim() || !questionTextDraft.trim()}>Добавить вопрос</Button></DialogFooter></DialogContent></Dialog>

      <Dialog open={standaloneDecisionOpen} onOpenChange={setStandaloneDecisionOpen}><DialogContent><DialogHeader><DialogTitle>Добавить решение без вопроса</DialogTitle><DialogDescription>Зафиксируйте решение, принятое во время ревью.</DialogDescription></DialogHeader><div className="space-y-4"><div className="space-y-2"><Label htmlFor="decision-title">Заголовок</Label><Input id="decision-title" value={decisionTitleDraft} onChange={(event) => setDecisionTitleDraft(event.target.value)} /></div><div className="space-y-2"><Label htmlFor="decision-text">Решение</Label><Textarea id="decision-text" value={decisionDraft} onChange={(event) => setDecisionDraft(event.target.value)} rows={4} className="text-slate-950 placeholder:text-slate-400" /></div></div><DialogFooter><Button variant="outline" onClick={() => setStandaloneDecisionOpen(false)}>Отмена</Button><Button onClick={addStandaloneDecision} disabled={!decisionTitleDraft.trim() || !decisionDraft.trim()}>Сохранить решение</Button></DialogFooter></DialogContent></Dialog>

      <Dialog open={Boolean(peopleInitiativeId)} onOpenChange={(open) => !open && setPeopleInitiativeId(null)}><DialogContent className="sm:max-w-2xl"><DialogHeader><DialogTitle>Состав и FTE инициативы</DialogTitle><DialogDescription>{peopleInitiative?.title}. Один добавленный сотрудник учитывается как 1,0 FTE.</DialogDescription></DialogHeader><div className="flex items-center justify-between rounded-xl bg-blue-50 px-4 py-3"><span className="text-sm font-medium text-blue-900">Текущий ресурс</span><span className="text-lg font-bold text-blue-800">{peopleInitiative?.fte.toFixed(1).replace(".", ",") ?? "0,0"} FTE</span></div><div className="grid max-h-[390px] gap-2 overflow-y-auto sm:grid-cols-2">{employeeDirectory.map((employee) => { const selected = peopleInitiative?.employees.includes(employee); return <button key={employee} onClick={() => peopleInitiativeId && toggleEmployee(peopleInitiativeId, employee)} className={`flex items-center gap-3 rounded-xl border p-3 text-left transition ${selected ? "border-blue-200 bg-blue-50" : "border-slate-200 bg-white hover:bg-slate-50"}`}><span className={`grid size-8 place-items-center rounded-full text-xs font-semibold ${selected ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-600"}`}>{employee.split(" ").map((part) => part[0]).join("")}</span><span className="min-w-0 flex-1 text-sm font-medium text-slate-800">{employee}</span><span className={`text-xs font-semibold ${selected ? "text-rose-600" : "text-blue-700"}`}>{selected ? "Удалить" : "Добавить"}</span></button>})}</div><DialogFooter><Button onClick={() => setPeopleInitiativeId(null)}>Готово</Button></DialogFooter></DialogContent></Dialog>

      <Dialog open={Boolean(riskInitiativeId)} onOpenChange={(open) => !open && setRiskInitiativeId(null)}><DialogContent><DialogHeader><DialogTitle>Добавить риск</DialogTitle><DialogDescription>{riskInitiative?.title}. Укажите уровень и опишите влияние.</DialogDescription></DialogHeader><div className="space-y-4"><div className="space-y-2"><Label htmlFor="risk-name">Имя риска</Label><Input id="risk-name" value={riskName} onChange={(event) => setRiskName(event.target.value)} /></div><div className="space-y-2"><Label>Уровень риска</Label><Select value={riskLevel} onValueChange={(value) => setRiskLevel(value as RiskLevel)}><SelectTrigger className="w-full"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="low">Низкий</SelectItem><SelectItem value="medium">Средний</SelectItem><SelectItem value="high">Высокий</SelectItem></SelectContent></Select></div><div className="space-y-2"><Label htmlFor="risk-description">Описание риска</Label><Textarea id="risk-description" value={riskDescription} onChange={(event) => setRiskDescription(event.target.value)} rows={4} /></div></div><DialogFooter><Button variant="outline" onClick={() => setRiskInitiativeId(null)}>Отмена</Button><Button onClick={addRisk} disabled={!riskName.trim() || !riskDescription.trim()}><ShieldAlert />Добавить риск</Button></DialogFooter></DialogContent></Dialog>
...
function QbrResults({ metricItems, initiativeItems, qbrInitiativeIds, questions }: { metricItems: MetricItem[]; initiativeItems: InitiativeItem[]; qbrInitiativeIds: string[]; questions: QbrQuestion[] }) {
  const selectedInitiatives = initiativeItems.filter((initiative) => qbrInitiativeIds.includes(initiative.id))
  const decisions = questions.filter((item) => item.decision.trim())
  return <main className="mx-auto w-full max-w-[1480px] p-4 md:p-7"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-2xl font-semibold text-slate-950">Итоги квартального ревью</h2><p className="mt-1 text-sm text-slate-500">Компактный отчет по результатам, инициативам и принятым решениям.</p></div><Button onClick={() => window.print()} className="bg-slate-950 text-white hover:bg-slate-800"><FileDown />PDF</Button></div><section className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><StatCard icon={Target} label="Метрики выполнены" value={`${metricItems.filter((metric) => metric.progress >= 100).length} / ${metricItems.length}`} note="по итогам периода" tone="blue" /><StatCard icon={Rocket} label="Инициативы завершены" value={`${selectedInitiatives.filter((item) => item.status === "Завершена").length} / ${selectedInitiatives.length}`} note="в портфеле QBR" tone="amber" /><StatCard icon={BriefcaseBusiness} label="Использовано ресурсов" value={`${selectedInitiatives.reduce((sum, item) => sum + item.fte, 0)} FTE`} note="командный ресурс" tone="violet" /><StatCard icon={ListChecks} label="Решения" value={`${decisions.length}`} note="зафиксировано на ревью" tone="red" /></section><div className="mt-5 grid gap-5 xl:grid-cols-[1.15fr_0.85fr]"><section className="rounded-2xl border border-slate-200 bg-white p-5"><h3 className="text-xl font-semibold text-slate-950">Результаты по метрикам</h3><div className="mt-4 space-y-4">{metricItems.map((metric) => <div key={metric.id}><div className="flex items-end justify-between gap-3"><div><p className="text-sm font-medium text-slate-900">{metric.name}</p><p className="mt-1 text-xs text-slate-500">Цель {metric.plan} · факт {metric.fact}</p></div><span className="text-sm font-semibold text-slate-800">{metric.progress}%</span></div><Progress value={Math.min(metric.progress, 100)} className="mt-2 h-2 bg-slate-100 [&_[data-slot=progress-indicator]]:bg-slate-900" /></div>)}</div></section><section className="rounded-2xl border border-slate-200 bg-white p-5"><h3 className="text-xl font-semibold text-slate-950">Принятые решения</h3><div className="mt-4 space-y-3">{decisions.length ? decisions.map((item) => <div key={item.id} className="rounded-xl bg-slate-50 p-4"><p className="text-sm font-semibold text-slate-900">{item.title}</p><p className="mt-2 text-sm leading-6 text-slate-600">{item.decision}</p></div>) : <p className="text-sm text-slate-500">Решения еще не зафиксированы.</p>}</div></section></div></main>
}
...
export default function Home() {
  ...
  const [questions, setQuestions] = useState<QbrQuestion[]>(() => initialQbrQuestions.map((item) => ({ ...item })))
  const quarter = quarters[quarterIndex]

  const pageTitle = globalPage === "Стратегия" ? "Стратегия Группы Московская Биржа" : globalPage === "Мои QBR" ? qbrName : globalPage
  const pageSubtitle = globalPage === "Стратегия" ? "Направления, драйверы и ключевые метрики группы компаний" : globalPage === "Метрики" ? "Единый каталог показателей" : globalPage === "Инициативы" ? "Работы, прогресс и статусы" : globalPage === "Команды" ? "Команды, подразделения и сотрудники" : "Квартальный обзор команды"
  const modeDescription = mode === "Подготовка" ? "Добавляйте метрики, инициативы и вопросы, обновляйте фактические значения, состав и риски." : mode === "Ревью" ? "Обсуждайте результаты и риски, фиксируйте решения по вопросам или отдельно от них." : "Проверьте основные результаты и решения, затем сохраните итоговый отчет."
  ...
  return (
    <SidebarProvider>
      <Sidebar className="border-r-0 bg-[#111827] text-white" collapsible="icon">
        <SidebarHeader className="border-b border-white/10 px-3 py-4"><div className="flex h-10 items-center gap-3 px-1"><img src="https://s.rbk.ru/v1_companies_s3/media/trademarks/d9413144-9ec6-48aa-ab15-570e4cdbaa3f.jpg" alt="MOEX" className="size-9 shrink-0 rounded-lg bg-white object-contain" /><div className="min-w-0 group-data-[collapsible=icon]:hidden"><p className="truncate text-sm font-semibold tracking-tight">MOEX QBR Tool</p><p className="truncate text-[10px] uppercase tracking-[0.16em] text-slate-400">Strategy & review</p></div></div></SidebarHeader>
        <SidebarContent className="bg-[#111827]">
          <SidebarGroup><SidebarGroupLabel className="text-slate-500">Управление результатами</SidebarGroupLabel><SidebarGroupContent><SidebarMenu>{navItems.map(({ label, icon: Icon }) => <SidebarMenuItem key={label}><SidebarMenuButton onClick={() => setGlobalPage(label)} isActive={globalPage === label} tooltip={label} className="h-10 text-slate-300 hover:bg-white/8 hover:text-white data-[active=true]:bg-white/10 data-[active=true]:text-white"><Icon /><span>{label}</span></SidebarMenuButton></SidebarMenuItem>)}</SidebarMenu></SidebarGroupContent></SidebarGroup>
          <SidebarGroup><SidebarGroupLabel className="text-slate-500">Доступные мне QBR</SidebarGroupLabel><SidebarGroupContent><SidebarMenu className="gap-1">{[{ name: "Цифровые решения", owner: "Алексей Иванов", period: quarter.short, status: mode }, { name: "Регистрация и онбординг", owner: "Ольга Морозова", period: "III кв. 2026", status: "Ревью" }].map((review) => <SidebarMenuItem key={review.name}><SidebarMenuButton onClick={() => openQbr(review.name, review.status)} className="h-auto rounded-xl border border-white/10 bg-white/5 p-2 text-white hover:bg-white/8"><span className="grid min-w-0 flex-1 grid-cols-[minmax(0,1fr)_auto] items-center gap-2 group-data-[collapsible=icon]:hidden"><span className="min-w-0"><span className="block truncate text-xs font-medium">{review.name}</span><span className="mt-0.5 block truncate text-[10px] text-slate-400">{review.owner}</span></span><span className="text-right"><span className="block whitespace-nowrap text-[10px] text-slate-400">{review.period}</span><span className={`mt-0.5 inline-flex rounded-full px-1.5 py-0.5 text-[9px] font-semibold ${review.status === "Итоги" ? "bg-emerald-400/15 text-emerald-300" : review.status === "Ревью" ? "bg-violet-400/15 text-violet-300" : "bg-amber-400/15 text-amber-300"}`}>{review.status}</span></span></span></SidebarMenuButton></SidebarMenuItem>)}</SidebarMenu></SidebarGroupContent></SidebarGroup>
        </SidebarContent>
      ...
      <SidebarInset className="min-w-0 bg-[#f5f7fa]">
        <header className="sticky top-0 z-20 flex min-h-[84px] items-center justify-between gap-3 border-b border-slate-200 bg-white/95 px-4 py-3 backdrop-blur md:px-7"><div className="flex min-w-0 items-center gap-3"><SidebarTrigger className="md:hidden" />{globalPage === "Мои QBR" ? <div className="min-w-0"><Select value={qbrName} onValueChange={setQbrName}><SelectTrigger className="h-10 w-[280px] border-0 bg-transparent px-0 text-xl font-semibold text-slate-950 shadow-none [&_[data-slot=select-value]]:text-slate-950 md:text-2xl"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="Цифровые решения">Цифровые решения</SelectItem><SelectItem value="Регистрация и онбординг">Регистрация и онбординг</SelectItem></SelectContent></Select><p className="mt-0.5 text-sm text-slate-500">{pageSubtitle}</p></div> : <div className="min-w-0"><h1 className="truncate text-xl font-semibold tracking-tight text-slate-950 md:text-2xl">{pageTitle}</h1><p className="mt-0.5 truncate text-sm text-slate-500">{pageSubtitle}</p></div>}</div><div className="flex shrink-0 items-center gap-2">{globalPage === "Мои QBR" && <QuarterSwitcher index={quarterIndex} onChange={setQuarterIndex} />}{globalPage === "Стратегия" && <Button variant="outline" size="sm" asChild className="hidden border-slate-300 bg-white font-semibold text-slate-900 hover:bg-slate-100 sm:flex"><a href="https://www.moex.com/files/4g62xymgykeh5zb9r40newqv42" target="_blank" rel="noreferrer">Источник <ExternalLink /></a></Button>}</div></header>
        ...
        {globalPage === "Инициативы" && <InitiativesPage items={initiativeItems} setItems={setInitiativeItems} metricCatalog={metricCatalog} teams={teams} />}
        {globalPage === "Команды" && <TeamsPage teams={teams} setTeams={setTeams} />}
        {globalPage === "Мои QBR" && <>
          <section className="border-b border-slate-200 bg-white px-4 py-4 md:px-7"><div className="mx-auto flex w-full max-w-[1480px] flex-wrap items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-slate-50 px-5 py-4"><div className="flex items-start gap-3">{mode === "Подготовка" ? <Pencil className="mt-1 size-5 text-blue-600" /> : mode === "Ревью" ? <ListChecks className="mt-1 size-5 text-violet-600" /> : <CheckCircle2 className="mt-1 size-5 text-emerald-600" />}<div><h2 className={`text-2xl font-semibold ${mode === "Подготовка" ? "text-blue-800" : mode === "Ревью" ? "text-violet-800" : "text-emerald-800"}`}>{mode}</h2><p className="mt-1 max-w-3xl text-sm leading-6 text-slate-600">{modeDescription}</p></div></div><div>{mode === "Подготовка" && <div className="flex min-w-[210px] flex-col items-stretch gap-1.5"><Button onClick={() => setMode("Ревью")} className="bg-blue-600 text-white hover:bg-blue-700">Начать ревью <ArrowRight className="size-5" /></Button><p className="text-center text-xs font-medium text-slate-500">Готовность к ревью: {quarter.completion}%</p></div>}{mode === "Ревью" && <div className="flex flex-wrap items-center justify-end gap-2"><Button onClick={() => setMode("Подготовка")} variant="outline" className="border-slate-300 bg-white text-slate-600 hover:bg-slate-100"><ArrowLeft className="size-5" />Вернуть в подготовку</Button><Button onClick={() => setMode("Итоги")} className="bg-violet-600 text-white hover:bg-violet-700">Завершить ревью <ArrowRight className="size-5" /></Button></div>}{mode === "Итоги" && <div className="flex flex-wrap items-center justify-end gap-2"><Button onClick={() => setMode("Ревью")} variant="outline" className="border-slate-300 bg-white text-slate-600 hover:bg-slate-100"><ArrowLeft className="size-5" />Вернуть на ревью</Button><Button onClick={() => window.print()} className="bg-slate-950 text-white hover:bg-slate-800"><FileDown />PDF</Button></div>}</div></div></section>
          {mode !== "Итоги" && <QbrPeriodOverview quarterIndex={quarterIndex} metricItems={metricItems} initiativeItems={initiativeItems} qbrInitiativeIds={qbrInitiativeIds} />}
          {mode === "Итоги" ? <QbrResults metricItems={metricItems} initiativeItems={initiativeItems} qbrInitiativeIds={qbrInitiativeIds} questions={questions} /> : <QbrOverview mode={mode} metricItems={metricItems} setMetricItems={setMetricItems} metricCatalog={metricCatalog} initiativeItems={initiativeItems} setInitiativeItems={setInitiativeItems} qbrInitiativeIds={qbrInitiativeIds} setQbrInitiativeIds={setQbrInitiativeIds} teams={teams} questions={questions} setQuestions={setQuestions} onOpenInitiatives={() => setGlobalPage("Инициативы")} />}
        </>}
      </SidebarInset>
    </SidebarProvider>
  )
