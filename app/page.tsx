  ...
  return (
    <SidebarProvider>
      <Sidebar className="border-r-0 bg-[#111827] text-white" collapsible="icon">
        <SidebarHeader className="border-b border-white/10 px-3 py-4"><div className="flex h-10 items-center gap-3 px-1"><div className="grid size-9 shrink-0 place-items-center rounded-lg border border-white/15 bg-white text-[9px] font-black tracking-[-0.08em] text-[#ef3e42]">MOEX</div><div className="min-w-0 group-data-[collapsible=icon]:hidden"><p className="truncate text-sm font-semibold tracking-tight">MOEX QBR Tool</p><p className="truncate text-[10px] uppercase tracking-[0.16em] text-slate-400">Strategy & review</p></div></div></SidebarHeader>
        <SidebarContent className="bg-[#111827]">
          ...
          <SidebarGroup><SidebarGroupLabel className="text-slate-500">Доступные мне QBR</SidebarGroupLabel><SidebarGroupContent><SidebarMenu className="gap-1.5">{[{ name: "Цифровые решения", owner: "Алексей Иванов", period: quarter.short, status: mode, color: mode === "Итоги" ? "bg-emerald-400" : mode === "Ревью" ? "bg-violet-400" : "bg-amber-400" }, { name: "Регистрация и онбординг", owner: "Ольга Морозова", period: "III кв. 2026", status: "Ревью", color: "bg-violet-400" }].map((review) => <SidebarMenuItem key={review.name}><SidebarMenuButton onClick={() => openQbr(review.name, review.status)} className="h-auto items-start rounded-xl border border-white/10 bg-white/5 p-2.5 text-white hover:bg-white/8"><span className={`mt-1 size-2 shrink-0 rounded-full ${review.color}`} /><span className="min-w-0 flex-1 group-data-[collapsible=icon]:hidden"><span className="block truncate text-xs font-medium">{review.name}</span><span className="mt-1 block truncate text-[10px] text-slate-400">{review.owner}</span><span className="mt-1.5 flex items-center justify-between gap-2 text-[10px]"><span className="text-slate-400">{review.period}</span><span className="rounded-full bg-white/10 px-2 py-0.5 font-medium text-slate-200">{review.status}</span></span></span></SidebarMenuButton></SidebarMenuItem>)}</SidebarMenu></SidebarGroupContent></SidebarGroup>
        </SidebarContent>
      ...
      <SidebarInset className="min-w-0 bg-[#f5f7fa]">
        ...
        {globalPage === "Мои QBR" && <>
          <section className="border-b border-slate-200 bg-white px-4 py-4 md:px-7"><div className="mx-auto flex w-full max-w-[1480px] flex-wrap items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-slate-50 px-5 py-4"><div className="flex items-start gap-3">{mode === "Подготовка" ? <Pencil className="mt-1 size-5 text-blue-600" /> : mode === "Ревью" ? <ListChecks className="mt-1 size-5 text-violet-600" /> : <CheckCircle2 className="mt-1 size-5 text-emerald-600" />}<div><div className="flex items-center gap-2"><span className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-400">Статус QBR</span><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${mode === "Подготовка" ? "bg-blue-100 text-blue-700" : mode === "Ревью" ? "bg-violet-100 text-violet-700" : "bg-emerald-100 text-emerald-700"}`}>{mode}</span></div><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">{modeDescription}</p></div></div><div className="flex min-w-[180px] flex-col items-stretch gap-1">{mode === "Подготовка" && <Button onClick={() => setMode("Ревью")} className="bg-blue-600 text-white hover:bg-blue-700">Начать ревью <ArrowRight /></Button>}{mode === "Ревью" && <Button onClick={() => setMode("Итоги")} className="bg-violet-600 text-white hover:bg-violet-700">Завершить ревью <Check /></Button>}{mode === "Итоги" && <Button onClick={() => window.print()} className="bg-slate-950 text-white hover:bg-slate-800"><FileDown />PDF</Button>}<button onClick={() => setMode("Подготовка")} className="px-2 py-1 text-xs text-slate-500 underline-offset-4 hover:text-slate-900 hover:underline">Вернуть в подготовку</button></div></div></section>
