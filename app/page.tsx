...
function MetricsPage({ items, setItems }: { items: MetricItem[]; setItems: (items: MetricItem[]) => void }) {
  ...
  const addMetric = () => {
    if (!nameDraft.trim() || !goalDraft || sourceMode !== "manual" || !valueDraft.trim()) return
    setItems([...items, { id: `M-${110 + items.length}`, name: nameDraft.trim(), strategy: "Стратегия компании", goal: goalDraft, plan: valueDraft.trim(), fact: valueDraft.trim(), progress: 100, status: "green", trend: "Новая", initiatives: 0, source: "Вручную" }])
    setDialogOpen(false); setNameDraft(""); setGoalDraft(""); setValueDraft(""); setSourceMode("manual")
  }
  return <main className="mx-auto w-full max-w-[1380px] p-4 md:p-7"><section className="overflow-hidden rounded-2xl border border-slate-200 bg-white"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4"><div><h2 className="text-base font-semibold text-slate-950">Справочник метрик</h2><p className="mt-1 text-xs text-slate-500">Метрики используются в стратегических целях, инициативах и квартальных обзорах.</p></div><Button onClick={() => setDialogOpen(true)} className="bg-[#ef3e42] text-white hover:bg-[#d92f34]"><Plus />Добавить метрику</Button></div><div className="overflow-x-auto"><Table className="min-w-[920px]"><TableHeader><TableRow className="bg-slate-50/70 hover:bg-slate-50/70"><TableHead className="w-[28%] pl-5 text-xs text-slate-500">Метрика</TableHead><TableHead className="w-[36%] text-xs text-slate-500">Стратегия</TableHead><TableHead className="w-[18%] text-xs text-slate-500">Значение</TableHead><TableHead className="w-[18%] pr-5 text-xs text-slate-500">Источник</TableHead></TableRow></TableHeader><TableBody>{items.map((metric) => <TableRow key={metric.id} className="border-slate-100"><TableCell className="py-4 pl-5"><p className="font-medium text-slate-900">{metric.name}</p><p className="mt-1 text-xs text-slate-400">{metric.id}</p></TableCell><TableCell><p className="text-sm font-medium text-slate-700">{metric.goal}</p><p className="mt-1 text-xs text-slate-400">{metric.strategy}</p></TableCell><TableCell><p className="text-lg font-semibold text-slate-950">{metric.fact}</p><p className="mt-1 text-xs font-medium text-emerald-600">{metric.trend}</p></TableCell><TableCell className="pr-5"><span className="inline-flex items-center gap-1.5 rounded-lg bg-slate-100 px-2.5 py-1.5 text-xs font-medium text-slate-700"><Database className="size-3.5" />{metric.source}</span></TableCell></TableRow>)}</TableBody></Table></div></section><Dialog open={dialogOpen} onOpenChange={setDialogOpen}><DialogContent><DialogHeader><DialogTitle>Добавить метрику</DialogTitle><DialogDescription>Создайте метрику и привяжите ее к стратегической цели.</DialogDescription></DialogHeader><div className="space-y-4"><div className="space-y-2"><Label htmlFor="new-metric-name">Метрика</Label><Input id="new-metric-name" value={nameDraft} onChange={(event) => setNameDraft(event.target.value)} placeholder="Название метрики" /></div><div className="space-y-2"><Label>Стратегия</Label><Select value={goalDraft} onValueChange={setGoalDraft}><SelectTrigger className="w-full"><SelectValue placeholder="Выберите стратегическую цель" /></SelectTrigger><SelectContent>{strategicGoals.map((goal) => <SelectItem key={goal} value={goal}>{goal}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><Label>Значение и источник</Label><Tabs value={sourceMode} onValueChange={setSourceMode}><TabsList className="grid h-10 w-full grid-cols-2"><TabsTrigger value="manual">Ручной ввод</TabsTrigger><TabsTrigger value="integration">SQL / DWH / ETL / API</TabsTrigger></TabsList><TabsContent value="manual" className="mt-3"><Input value={valueDraft} onChange={(event) => setValueDraft(event.target.value)} placeholder="Например: 18%, 50 тыс. или 7 дней" /></TabsContent><TabsContent value="integration" className="mt-3 rounded-xl border border-dashed border-slate-300 bg-slate-50 p-5 text-sm leading-6 text-slate-500">Возможность добавить источник будет реализована в следующих версиях приложения.</TabsContent></Tabs></div></div><DialogFooter><Button variant="outline" onClick={() => setDialogOpen(false)}>Отмена</Button><Button onClick={addMetric} disabled={!nameDraft.trim() || !goalDraft || sourceMode !== "manual" || !valueDraft.trim()}>Добавить</Button></DialogFooter></DialogContent></Dialog></main>
}
...
function TeamsPage({ teams, setTeams }: { teams: TeamItem[]; setTeams: (teams: TeamItem[]) => void }) {
  ...
  return <main className="mx-auto w-full max-w-[1480px] p-4 md:p-7"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-semibold text-slate-950">Команды и сотрудники</h2><p className="mt-1 text-sm text-slate-500">Справочник используется для назначения владельцев и расчета FTE инициатив.</p></div><div className="flex flex-wrap gap-2"><Button disabled className="border border-slate-300 bg-slate-200 text-slate-500 disabled:opacity-100"><Upload />Импорт из службы каталогов</Button><Button onClick={() => setTeamDialogOpen(true)} className="bg-[#ef3e42] text-white hover:bg-[#d92f34]"><Plus />Добавить команду</Button></div></div><section className="mt-5 overflow-hidden rounded-2xl border border-slate-200 bg-white"><div className="overflow-x-auto"><Table className="min-w-[980px]"><TableHeader><TableRow className="bg-slate-50/80 hover:bg-slate-50/80"><TableHead className="w-[24%] pl-5 text-xs text-slate-500">Команда</TableHead><TableHead className="w-[20%] text-xs text-slate-500">Подразделение</TableHead><TableHead className="w-[12%] text-center text-xs text-slate-500">Сотрудников</TableHead><TableHead className="w-[30%] text-xs text-slate-500">Состав команды</TableHead><TableHead className="w-[14%] pr-5 text-right text-xs text-slate-500">Действия</TableHead></TableRow></TableHeader><TableBody>{teams.map((team) => <TableRow key={team.id} className="border-slate-100 align-top"><TableCell className="py-5 pl-5"><div className="flex items-center gap-3"><span className="rounded-xl bg-blue-50 p-2.5 text-blue-700"><Building2 className="size-5" /></span><div><p className="font-semibold text-slate-950">{team.name}</p><p className="mt-1 text-xs text-slate-400">{team.id}</p></div></div></TableCell><TableCell className="py-5 text-sm text-slate-600">{team.department}</TableCell><TableCell className="py-5 text-center"><span className="text-3xl font-semibold tracking-tight text-slate-950">{team.employees.length}</span><p className="mt-1 text-[11px] text-slate-400">человек</p></TableCell><TableCell className="py-5"><div className="space-y-2">{team.employees.length ? team.employees.map((employee) => <div key={employee.id} className="flex items-center gap-3 rounded-xl bg-slate-50 px-3 py-2"><span className="grid size-8 shrink-0 place-items-center rounded-full bg-white text-[10px] font-semibold text-slate-600 ring-1 ring-slate-200">{employee.name.split(" ").map((part) => part[0]).join("")}</span><div className="min-w-0"><p className="truncate text-xs font-medium text-slate-800">{employee.name} · {employee.role}</p><p className="mt-0.5 truncate text-[11px] text-slate-400">{employee.email}</p></div></div>) : <span className="text-sm text-slate-400">Сотрудники еще не добавлены</span>}</div></TableCell><TableCell className="py-5 pr-5 text-right"><Button onClick={() => setEmployeeTeamId(team.id)} size="sm" className="bg-blue-600 text-white hover:bg-blue-700"><UserPlus />Добавить сотрудника</Button></TableCell></TableRow>)}</TableBody></Table></div></section><Dialog open={teamDialogOpen} onOpenChange={setTeamDialogOpen}><DialogContent><DialogHeader><DialogTitle>Добавить команду</DialogTitle><DialogDescription>Создайте команду в организационном справочнике.</DialogDescription></DialogHeader><div className="space-y-4"><div className="space-y-2"><Label htmlFor="team-name">Название команды</Label><Input id="team-name" value={teamName} onChange={(event) => setTeamName(event.target.value)} /></div><div className="space-y-2"><Label htmlFor="team-department">Подразделение</Label><Input id="team-department" value={department} onChange={(event) => setDepartment(event.target.value)} /></div></div><DialogFooter><Button variant="outline" onClick={() => setTeamDialogOpen(false)}>Отмена</Button><Button onClick={addTeam} disabled={!teamName.trim() || !department.trim()}>Добавить команду</Button></DialogFooter></DialogContent></Dialog><Dialog open={Boolean(employeeTeamId)} onOpenChange={(open) => !open && setEmployeeTeamId(null)}><DialogContent><DialogHeader><DialogTitle>Добавить сотрудника</DialogTitle><DialogDescription>Команда: {selectedTeam?.name}</DialogDescription></DialogHeader><div className="space-y-4"><div className="space-y-2"><Label htmlFor="employee-name">Имя</Label><Input id="employee-name" value={employeeName} onChange={(event) => setEmployeeName(event.target.value)} /></div><div className="space-y-2"><Label htmlFor="employee-role">Должность</Label><Input id="employee-role" value={employeeRole} onChange={(event) => setEmployeeRole(event.target.value)} /></div><div className="space-y-2"><Label htmlFor="employee-email">Email</Label><Input id="employee-email" type="email" value={employeeEmail} onChange={(event) => setEmployeeEmail(event.target.value)} /></div></div><DialogFooter><Button variant="outline" onClick={() => setEmployeeTeamId(null)}>Отмена</Button><Button onClick={addEmployee} disabled={!employeeName.trim() || !employeeRole.trim() || !employeeEmail.trim()} className="bg-blue-600 text-white hover:bg-blue-700">Добавить сотрудника</Button></DialogFooter></DialogContent></Dialog></main>
}
...
function QbrOverview({ mode, metricItems, setMetricItems, metricCatalog, initiativeItems, setInitiativeItems, qbrInitiativeIds, setQbrInitiativeIds, teams, onOpenInitiatives }: { quarterIndex?: number; mode: string; metricItems: MetricItem[]; setMetricItems: (items: MetricItem[]) => void; metricCatalog: MetricItem[]; initiativeItems: InitiativeItem[]; setInitiativeItems: (items: InitiativeItem[]) => void; qbrInitiativeIds: string[]; setQbrInitiativeIds: (ids: string[]) => void; teams: TeamItem[]; onOpenInitiatives: () => void }) {
  ...
  return (
    <main className="mx-auto w-full max-w-[1480px] p-4 md:p-7">
      ...
      <section className="mt-5 overflow-hidden rounded-2xl border border-slate-200 bg-white"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4"><div><div className="flex items-center gap-2"><ListChecks className="size-4 text-[#ef3e42]" /><h3 className="text-sm font-semibold text-slate-950">Вопросы и решения</h3></div><p className="mt-1 text-xs text-slate-500">Риски, сложности и ограничения для обсуждения на QBR с фиксацией решения</p></div>{editable && <Button size="sm" className="bg-slate-950 text-white hover:bg-slate-800"><Plus />Добавить вопрос</Button>}</div><div className="divide-y divide-slate-100">{workspaceData["Вопросы и решения"].items.map((item, index) => <article key={item.title} className="grid gap-4 p-5 lg:grid-cols-[36px_minmax(0,1fr)_150px] lg:items-start"><span className="grid size-8 place-items-center rounded-full bg-slate-100 text-xs font-semibold text-slate-600">{index + 1}</span><div><div className="flex flex-wrap items-center gap-2"><h4 className="font-semibold text-slate-950">{item.title}</h4><StatusPill label={item.value} tone={item.tone} /></div><p className="mt-1 text-xs text-slate-500">{item.meta}</p><div className="mt-3 grid gap-2 md:grid-cols-3">{item.details.map((detail) => <div key={detail} className="rounded-xl bg-slate-50 px-3 py-2.5 text-xs leading-5 text-slate-600">{detail}</div>)}</div></div><Button variant="outline" size="sm" className="justify-self-start lg:justify-self-end">{editable ? "Зафиксировать решение" : "Открыть решение"}</Button></article>)}</div></section>
...
export default function Home() {
  ...
  const pageSubtitle = globalPage === "Стратегия" ? "Стратегия 2028 → цели и метрики → квартальные инициативы" : globalPage === "Мои QBR" ? "Квартальный обзор команды" : "Рабочее пространство"
  ...
  return (
    <SidebarProvider>
      ...
      <SidebarInset className="min-w-0 bg-[#f5f7fa]">
        {(globalPage === "Метрики" || globalPage === "Инициативы") && <header className="flex h-12 items-center border-b border-slate-200 bg-white px-4 md:hidden"><SidebarTrigger /></header>}
        {globalPage !== "Метрики" && globalPage !== "Инициативы" && <header className="sticky top-0 z-20 flex min-h-[72px] items-center justify-between gap-3 border-b border-slate-200 bg-white/95 px-4 py-2 backdrop-blur md:px-7"><div className="flex min-w-0 items-center gap-3"><SidebarTrigger className="md:hidden" />{globalPage === "Мои QBR" && <QuarterSwitcher index={quarterIndex} onChange={setQuarterIndex} />}{globalPage === "Мои QBR" ? <div className="min-w-0"><Select value={qbrName} onValueChange={setQbrName}><SelectTrigger className="h-9 w-[250px] border-0 bg-transparent px-1 text-base font-semibold shadow-none"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="Цифровые решения">Цифровые решения</SelectItem><SelectItem value="Регистрация и онбординг">Регистрация и онбординг</SelectItem></SelectContent></Select><p className="px-1 text-xs text-slate-500">{pageSubtitle}</p></div> : <div className="hidden min-w-0 lg:block"><h1 className="truncate text-sm font-semibold text-slate-950 md:text-base">{pageTitle}</h1><p className="truncate text-xs text-slate-500">{pageSubtitle}</p></div>}</div><div className="flex shrink-0 items-center gap-2">{globalPage === "Стратегия" && <Button variant="outline" size="sm" asChild className="hidden sm:flex"><a href="https://www.moex.com/files/4g62xymgykeh5zb9r40newqv42" target="_blank" rel="noreferrer">Источник <ExternalLink /></a></Button>}</div></header>}
        ...
        {globalPage === "Мои QBR" && <><div className="border-b border-slate-200 bg-white px-4 py-4 md:px-7"><div className="mx-auto w-full max-w-[1480px]"><div className="grid w-full grid-cols-3 rounded-xl bg-slate-100 p-1">{["Подготовка", "Ревью", "Итоги"].map((item) => <button key={item} onClick={() => setMode(item)} className={`rounded-lg px-4 py-2.5 text-sm font-medium transition ${mode === item ? "bg-slate-950 text-white shadow-sm" : "text-slate-500 hover:text-slate-900"}`}>{item}</button>)}</div><div className={`mt-3 flex flex-wrap items-center gap-2 rounded-xl border px-4 py-3 text-xs ${mode === "Подготовка" ? "border-blue-200 bg-blue-50 text-blue-800" : mode === "Ревью" ? "border-violet-200 bg-violet-50 text-violet-800" : "border-emerald-200 bg-emerald-50 text-emerald-800"}`}>{mode === "Подготовка" ? <Pencil className="size-4" /> : mode === "Ревью" ? <ListChecks className="size-4" /> : <CheckCircle2 className="size-4" />}<span className="font-semibold">Режим {mode.toLowerCase()}:</span><span>{mode === "Подготовка" ? "добавляйте метрики и инициативы, обновляйте фактические значения, состав и риски." : mode === "Ревью" ? "обсуждайте результаты, риски и вопросы, фиксируйте решения по итогам встречи." : "проверьте принятые решения, зафиксируйте итоги периода и сформируйте итоговый отчет."}</span></div></div></div><QbrPeriodOverview quarterIndex={quarterIndex} metricItems={metricItems} initiativeItems={initiativeItems} qbrInitiativeIds={qbrInitiativeIds} /><QbrOverview quarterIndex={quarterIndex} mode={mode} metricItems={metricItems} setMetricItems={setMetricItems} metricCatalog={metricCatalog} initiativeItems={initiativeItems} setInitiativeItems={setInitiativeItems} qbrInitiativeIds={qbrInitiativeIds} setQbrInitiativeIds={setQbrInitiativeIds} teams={teams} onOpenInitiatives={() => setGlobalPage("Инициативы")} /></>}
      </SidebarInset>
    </SidebarProvider>
  )
}
